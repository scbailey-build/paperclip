...
describe("Sidebar", () => {
  ...
  async function renderSidebar() {
    ...
    // Work/Company start collapsed behind the four-item top nav, so open them
    // up front — most assertions below target the section contents.
    for (const label of ["Work", "Company"]) {
      const trigger = container.querySelector<HTMLButtonElement>(
        `button[aria-label="Expand ${label}"]`,
      );
      if (trigger) {
        flushSync(() => {
          trigger.click();
  ...
  it("renders plugin sidebar slots in Work below Workspaces", async () => {
    ...
    expect(primaryNavText).toContain("Brief");
  ...
  it("keeps the four-item top-level nav (Brief, Board, Workflows, Skills) outside Work", async () => {
    mockInstanceSettingsApi.getExperimental.mockResolvedValue({ enableIsolatedWorkspaces: false });
    const root = await renderSidebar();

    const primaryNav = container.querySelector("nav > div:first-child");
    const primaryLabels = [...(primaryNav?.querySelectorAll("a") ?? [])].map(
      (anchor) => anchor.textContent?.trim(),
    );
    expect(primaryLabels).toEqual(["Brief", "Board", "Workflows", "Skills"]);

    const skillsLink = [...(primaryNav?.querySelectorAll("a") ?? [])].find(
      (anchor) => anchor.textContent?.trim() === "Skills",
    );
    ...
    expect(workSection?.textContent).toContain("Artifacts");
